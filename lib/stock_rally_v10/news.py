...
def _gdelt_throttle():
    import time
    v = cfg.__dict__.get("cfg.GDELT_REQUEST_DELAY_SEC")
    if v is None:
        v = cfg.__dict__.get("cfg.NEWS_GDELT_SECONDS_PER_CALL", 6.0)
...
def _bq_resolve_project():
    ...
    p = cfg.__dict__.get("cfg.BQ_PROJECT_ID")
    if p:
...
def _bq_geo_sql(is_macro):
    ...
    if is_macro:
        mc = cfg.__dict__.get("cfg.BQ_MACRO_GEO_COUNTRIES")
    else:
        if not cfg.__dict__.get("cfg.BQ_SECTOR_USE_GEO_FILTER", True):
            return ""
        mc = cfg.__dict__.get("cfg.BQ_SECTOR_GEO_COUNTRIES")
    if not mc:
        return ""
...
def _bq_query_all_channels_gkg(a, b):
    ...
    if not cfg.__dict__.get("cfg.BQ_USE_PARTITION_FILTER", True):
        print(
            "[BigQuery] WARNUNG: cfg.BQ_USE_PARTITION_FILTER=False kann Full-Table-Scan bedeuten.",
            flush=True,
        )
    table = cfg.__dict__.get("cfg.GDELT_BQ_EVENTS_TABLE", "`gdelt-bq.gdeltv2.gkg_partitioned`")
...
def _bq_query_channel_date_range(channel, a, b):
    ...
    from google.cloud import bigquery
    use_gkg = cfg.__dict__.get("cfg.BQ_USE_GKG_TABLE", True)
    is_macro = channel == "macro"
    if is_macro:
        theme_sql = cfg.__dict__.get("cfg.MACRO_BQ_THEME_WHERE", "(1=1)")
    else:
    ...
    params = []
    partition_sql = ""
    if cfg.__dict__.get("cfg.BQ_USE_PARTITION_FILTER", True):
        partition_sql = (
...
"""
        ...
        tc = cfg.__dict__.get("cfg.BQ_THEMES_COLUMN", "V2Themes")
        theme_sql = theme_sql.replace("V2Themes", tc)
        dname = str(cfg.__dict__.get("cfg.BQ_EVENT_DATE_FIELD", "SQLDATE"))
...
def _fetch_news_sentiment_bigquery(df, start, end):
    ...
    cache_path = cfg.__dict__.get("cfg.NEWS_CACHE_FILE", os.path.join(os.getcwd(), "data", "news_gdelt_cache.pkl"))
    ...
    use_gkg = cfg.__dict__.get("cfg.BQ_USE_GKG_TABLE", True)
    single_scan = bool(cfg.__dict__.get("cfg.BQ_SINGLE_SCAN", True)) and use_gkg
    if n_ranges == 0:
        pass
    elif single_scan:
        print(
            f"[BigQuery] {n_ranges} Lücken-Bereiche über {len(channels)} Kanäle "
            f"→ 1 Query (Single-Scan, _PARTITIONTIME). Tabelle: {cfg.__dict__.get('cfg.GDELT_BQ_EVENTS_TABLE', '')}",
            flush=True,
        )
    else:
        print(
            f"[BigQuery] {n_ranges} Lücken-Bereiche über {len(channels)} Kanäle "
            f"(≈ {n_ranges} Queries; kein HTTP-429). Tabelle: {cfg.__dict__.get('cfg.GDELT_BQ_EVENTS_TABLE', '')}",
            flush=True,
        )
    print(
...
def _fetch_news_sentiment_gdelt_api(df, start=cfg.START_DATE, end=cfg.END_DATE, max_threads=8):
    ...
    import time as _time
    cache_path = cfg.__dict__.get("cfg.NEWS_CACHE_FILE", os.path.join(os.getcwd(), "data", "news_gdelt_cache.pkl"))
    sec_per_call = float(cfg.__dict__.get("cfg.NEWS_GDELT_SECONDS_PER_CALL", 6.0))
...
def fetch_news_sentiment(df, start=cfg.START_DATE, end=cfg.END_DATE, max_threads=8):
    ...
    ns = cfg.__dict__.get("cfg.NEWS_BQ_START_DATE")
    ne = cfg.__dict__.get("cfg.NEWS_BQ_END_DATE")
    if ns:
        start = ns
    else:
        start = price_start
        yb = int(cfg.__dict__.get("cfg.NEWS_EXTRA_HISTORY_YEARS_BEFORE", 0) or 0)
        if yb > 0:
    ...
    if ne:
        ...
        ya = int(cfg.__dict__.get("cfg.NEWS_EXTRA_HISTORY_YEARS_AFTER", 0) or 0)
        if ya > 0:
    ...
    if cfg.__dict__.get("cfg.NEWS_SOURCE", "bigquery") == "bigquery":
