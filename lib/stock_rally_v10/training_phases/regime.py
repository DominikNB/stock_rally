"""Phase 14: Marktregime-Auswertung (Benchmark + optional VIX) auf THRESHOLD-Set."""
...
def run_phase_regime_benchmark_report(cfg_mod: Any | None = None) -> None:
    if cfg_mod is None:
    ...
    def _yf_close_series(ticker: str):
        raw = yf.download(
            ticker,
            start=cfg_mod.START_DATE,
            end=cfg_mod.END_DATE,
            auto_adjust=True,
            threads=False,
            progress=False,
        )
    ...
    def _regime_table_from_close(close: pd.Series, name: str = "bench"):
    ...
    def _merge_regime_on_threshold(df_thr, regime_df, date_col: str = "Date"):
        left = df_thr.copy()
        ...
        out = pd.merge_asof(
            ...
            right,
            left_on="_d",
            right_on="_regime_date",
            direction="backward",
        )
        return out.drop(columns=["_d", "_regime_date"], errors="ignore")

    def _report_regime_metrics(
        df_ev, prob_col="prob", pred_col="pred", true_col="target", group_cols=None
    ):
    ...
    try:
        ...
        if _close_b is None:
            print("Regime-Evaluation: Benchmark-Download leer — übersprungen.", flush=True)
            return
        ...
        print("\n" + "=" * 60, flush=True)
        print("Regime-Evaluation (nur Bericht, THRESHOLD-Set)", flush=True)
        print(
            f"Benchmark: {REGIME_BENCHMARK}  |  thr={thr:.3f}  |  AP aus prob; Prec/Rec aus pred ≥ thr",
            flush=True,
        )
        print("=" * 60, flush=True)
        _by_y = _report_regime_metrics(_df_r, "prob", "pred", "target", ["year"])
        if len(_by_y):
            print("\nPro Kalenderjahr:", flush=True)
            print(_by_y.sort_values("year").to_string(index=False), flush=True)
        _cols = [c for c in ["mkt_trend", "mkt_vol"] if c in _df_r.columns]
        if _cols:
            print("\nPro Markt-Regime (Benchmark), Trend × Vol (alle Kombinationen):", flush=True)
            print(
                _report_regime_metrics(_df_r, "prob", "pred", "target", _cols).to_string(index=False),
                flush=True,
            )
        if "vix_bucket" in _df_r.columns:
            print("\nPro VIX-Bucket (vs. 252d-Median, kausal shift(1)):", flush=True)
            print(
                _report_regime_metrics(_df_r, "prob", "pred", "target", ["vix_bucket"]).to_string(
                    index=False
                ),
                flush=True,
            )
        ...
        print(f"Regime-Evaluation Fehler: {_e}", flush=True)
